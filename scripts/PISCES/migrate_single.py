...
# IT was developed because there was an oversight with a single HUC12 after running migrate_hucs.py - it needed to be corrected
# by running the code once more with a slightly different input. This is kept for legacy purposes but should NOT be used
...
migs_file = os.path.join(os.getcwd(), "utils", "huc12_updates_2013_rollback.csv")  # file containing all of the migrations
...
log_dir = os.path.join(os.getcwd(), "log", "huc_migration_2013_rollback", )
...
class huc_change():
	...
	def load_item(self, table, storage_list, db_cursor):
		...
		tcurs = self.db_conn.cursor()
		i = 0
		for record in records:
			log.write("OBJECTID: %s, Table: %s, iteration: %s" % (record.OBJECTID, table, i), 1)
			if not record.OBJECTID:
				log.write("Skipping because no objectid")
				continue
			observation = local_vars.observation()
			observation.load(record.OBJECTID, from_table=table, db_cursor=tcurs)
